...
'In testing, the batch size is too large to save in .mat file (batch size > 1000) -----> so we do not save the generated ""combined_A"", since only three individual channels will be used in the refining program! '
...
def main_generate_channel_forTesting_and_Refining(N_G, num_antenna_bs, num_elements_irs, num_user, irs_Nh, num_samples, Rician_factor, total_blocks,
                          scale_factor, rho_G=0.9995, rho_hr=0.9995, rho_hd=0.9995, location_bs=np.array([100, -100, 0]),
                          location_irs=np.array([0, 0, 0]), isTesting=False):
    """
        :save:  channel_G: array (num_samples, num_antenna_bs, num_elements_irs, total_blocks)
                channel_hr: array (num_samples, num_elements_irs, num_user, total_blocks)
                channel_hd: array (num_samples, num_antenna_bs, num_user, total_blocks)
    ...
    if not os.path.exists('Rician_channel'):
        os.makedirs('Rician_channel')
    params_all = (num_antenna_bs, num_elements_irs, num_user, irs_Nh, num_samples, Rician_factor, scale_factor, total_blocks)
    file_test_channel = './Rician_channel/channel_Test' + str(params_all) + '.mat'

    # we do not save 'combined_A' since its size too big ---> out of Memory!
    sio.savemat(file_test_channel, {'channel_bs_irs': channel_G, 'channel_irs_user': channel_hr, 'channel_bs_user': channel_hd})

    # 'This return is only for the Python code to output NN designed results. we still need the true channel to compute the downlink rate --> saved in .MAT file'
    return combined_A
