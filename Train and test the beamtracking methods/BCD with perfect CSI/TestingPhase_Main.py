...
from generate_channel import main_generate_channel_testing_rate, main_generate_channel
from Designed_Neural_Layers import MLP_input_w,MLP_input_F,MLPBlock0,MLPBlock1,MLPBlock2,MLPBlock3,MLPBlock4,GNN_layer
from Pilots_Generation import Contribution_receivedpilots
...
batch_size_test_1block = 1000 # 500 # in testing case in a pretrained model, it should be 10000
...
#######################################################################################################################
    ...
    # Generate the Beamformer using the transmit MMSE (Regularized ZF) beamforming
    F_complex = Generate_Beamformer(channel_combined_input[:, :, :, :, 0], w_her_complex, p_allocation_sqrt, lambda_allocation_linear, num_user, num_antenna_bs, Pt_down_linear, noise_power_linear_down)
...
for frame in range(frames):
    ...
    with tf.Session() as sess:
        ...
        best_loss, Minrate_block_test, w_fetch, F_fetch = sess.run([loss, Minrate_count, w_her_complex, F_complex_testing], feed_dict=feed_dict_val)
...
###########################################################################################################################
# Record the computed min use rate without estimating low-dim channel and re-design beamformer F.
...
###########################################################################################################################
'Estimating the channel using the LMMSE estimator'
'We first assume the perfect CSI, then apply the LMMSE with different pilot length.'

# combined_A_test: array (num_samples, num_antenna_bs, num_elements_irs, num_user, total_blocks), dtype=complex128
# We only need to estimate the channel at the first block of each frame
combined_A_pilot_stages = np.empty([batch_size_test_1block, num_antenna_bs, (num_elements_irs + 1), num_user, frames], dtype=complex)
for frame in range(frames):
    combined_A_pilot_stages[:, :, :, :, frame] = combined_A_test[:, :, :, :, frame * N_w_using]

# Pilots_all = [batch_size_test_1block, num_antenna_bs, U_repeat, num_user, frames], dtype=complex
# combined_A_pilot_stages: (batch_size_test_1block, num_antenna_bs, (num_elements_irs + 1), num_user, frames)
# w_noDirect_all = [batch_size_test_1block, num_elements_irs, 1, frames], dtype=complex
# w_withDirect_all = [batch_size_test_1block, (num_elements_irs + 1), 1, frames], dtype=complex

# Estimated_A = np.empty([batch_size_test_1block, num_antenna_bs, (num_elements_irs + 1), num_user, frames], dtype=complex)
...
for frame in range(frames):
    for kk in range(num_user):
        # A_kk = combined_A_pilot_stages[:, :, :, kk, frame] # A_kk: [batch_size_test_1block, num_antenna_bs, (num_elements_irs + 1)]
        # mean_A_kk = np.mean(A_kk, axis=0, keepdims=True) # mean_A_kk: [1, num_antenna_bs, (num_elements_irs + 1)]
        # ZeroMean_A_kk = A_kk - mean_A_kk # ZeroMean_A_kk: [batch_size_test_1block, num_antenna_bs, (num_elements_irs + 1)]
        #
        # Y_kk = Pilots_all[:, :, :, kk, frame] # Y_kk: [batch_size_test_1block, num_antenna_bs, U_repeat]
        # mean_Y_kk = np.mean(Y_kk, axis=0, keepdims=True) # mean_Y_kk: [1, num_antenna_bs, U_repeat]
        # ZeroMean_Y_kk = Y_kk - mean_Y_kk  # ZeroMean_Y_kk: [batch_size_test_1block, num_antenna_bs, U_repeat]
        #
        # # Compute the covariance matrix C_YY ----> Thanks to the Numpy broadcasting rule for matrix multiplication!
        # C_YY = np.sum(np.matmul(np.transpose(ZeroMean_Y_kk.conjugate(), (0, 2, 1)), ZeroMean_Y_kk), axis=0) / batch_size_test_1block # C_YY: [U_repeat, U_repeat]
        #
        # # Compute the covariance matrix C_YA ----> Thanks to the Numpy broadcasting rule for matrix multiplication!
        # C_YA = np.sum(np.matmul(np.transpose(ZeroMean_Y_kk.conjugate(), (0, 2, 1)), ZeroMean_A_kk), axis=0) / batch_size_test_1block  # C_YA: [U_repeat, (num_elements_irs + 1)]
        #
        # # Using the LMMSE estimator to estimate the Combined channel A
        # Estimated_A_tmp = ZeroMean_Y_kk @ np.linalg.inv(C_YY) # Estimated_A_tmp: [batch_size_test_1block, num_antenna_bs, U_repeat]
        # Estimated_A[:, :, :, kk, frame] = Estimated_A_tmp @ C_YA + mean_A_kk # [batch_size_test_1block, num_antenna_bs, (num_elements_irs + 1)]

        # Compute the channel gain using the estimated channel ----> as the input for the OLP algorithm
        # H_lmmse_designed_w[:, :, :, kk, frame] = Estimated_A[:, :, :, kk, frame] @ w_withDirect_all[:, :, :, frame] # [batch_size_test_1block, num_antenna_bs, 1]
        H_lmmse_designed_w[:, :, :, kk, frame] = combined_A_pilot_stages[:, :, :, kk, frame] @ w_withDirect_all[:, :, :, frame]  # [batch_size_test_1block, num_antenna_bs, 1]
