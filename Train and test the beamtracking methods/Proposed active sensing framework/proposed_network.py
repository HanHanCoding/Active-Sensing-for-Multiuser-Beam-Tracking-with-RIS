...
from Designed_Neural_Layers import MLP_input_w,MLP_input_F,MLPBlock_w_node,MLPBlock_V_node,MLPBlock1,MLPBlock2,MLPBlock3,MLPBlock4,MLPBlock5,MLPBlock6,GNN_layer,LSTM_model,MLP_input_V
...
n_epochs = 10000  # Num of epochs -----> controlling the early stopping
...
# this two placeholder is for LMMSE estimation, equivalent to the channel stats rather than the actual channels ---> so the same each frame!
...
with tf.name_scope("channel_sensing"):
    ...
    'GNN layers for {V, w, F}'
    'The GNN has two layers for spatial convolution'
    # first layer
    MLP_w_1, MLP_V_1, MLP1_1, MLP2_1, MLP3_1, MLP4_1, MLP5_1, MLP6_1 = MLPBlock_w_node(), MLPBlock_V_node(), MLPBlock1(), MLPBlock2(), MLPBlock3(), MLPBlock4(), MLPBlock5(), MLPBlock6()
    # second layer
    MLP_w_2, MLP_V_2, MLP1_2, MLP2_2, MLP3_2, MLP4_2, MLP5_2, MLP6_2 = MLPBlock_w_node(), MLPBlock_V_node(), MLPBlock1(), MLPBlock2(), MLPBlock3(), MLPBlock4(), MLPBlock5(), MLPBlock6()
    'The layer to generate the INPUT representative vector for the user node and RIS node!'
    MLP_w_in, MLP_V_in, MLP_F_in = MLP_input_w(), MLP_input_V(), MLP_input_F()

    'DNN for calculating the UL RIS sensing vector v for each frame'
    # DNN_V_mapping = DNN_mapping_V()
    DNN_V_mapping_output = Dense(units=2 * num_elements_irs * subblocks_L, activation='linear')  # the linear(ouput) layer of DNN_V_mapping
...
#######################################################################################################################
    ...
    for frame in range(frames):
        ...
        'generate the INPUT representative vector for the RIS node V'
        c_RIS_V = MLP_V_in(input_RIS)

        # Spatial convolution for the first time
        c_user, c_RIS_w, c_RIS_V = GNN_layer(c_user, c_RIS_w, c_RIS_V, MLP_w_1, MLP_V_1, MLP1_1, MLP2_1, MLP3_1, MLP4_1, MLP5_1, MLP6_1, num_user)
        # Spatial convolution for the second time
        F_UE_all, w_candidate, V_candidate = GNN_layer(c_user, c_RIS_w, c_RIS_V, MLP_w_2, MLP_V_2, MLP1_2, MLP2_2, MLP3_2, MLP4_2, MLP5_2, MLP6_2, num_user)
        ...
        'RIS sensing vectors V'
        'This V is the predicted V, used for pilot receiving in the next frame!'
        # V_her = DNN_V_mapping(RIS_phase_candidate)
        V_her = DNN_V_mapping_output(V_candidate)
        # V_subblocks_all = []
        for subblock in range(subblocks_L):
            # 'assign the unique sensing vector to each of the sub-block'
            real_v0 = tf.reshape(V_her[:, (0 + subblock * 2 * num_elements_irs):(num_elements_irs + subblock * 2 * num_elements_irs)], [batch_size, num_elements_irs])
            imag_v0 = tf.reshape(V_her[:, (num_elements_irs + subblock * 2 * num_elements_irs):(2 * num_elements_irs + subblock * 2 * num_elements_irs)], [batch_size, num_elements_irs])
            ...
            v_her_complex = tf.concat([tf.ones([batch_size, 1], tf.complex64), v_her_complex], 1)

            v_her_complex = tf.reshape(v_her_complex, [batch_size, (num_elements_irs + 1), 1])
            # V_subblocks_all: list[tensor(batch_size, (num_elements_irs + 1), 1),..., tensor(batch_size, (num_elements_irs + 1), 1)], in total subblocks_L items
            # V_subblocks_all.append(v_her_complex)
            if subblock == 0:
                V_subblocks_all = v_her_complex
            else:
                V_subblocks_all = tf.concat([V_subblocks_all, v_her_complex], axis=2)  # V_subblocks_all: tensor(batch_size, (num_elements_irs + 1), subblocks_L)
        ...
        if frame > 0:
            # we don't calculate the sumrate for frame 0 (the first LSTM cell), so we input the range of the blocks like this:
