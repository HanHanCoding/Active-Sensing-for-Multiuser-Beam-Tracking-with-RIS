...
batch_size_test_1block = 1 # in testing case in a pretrained model, it should be 10000
batch_size_train_1block = batch_size_test_1block

#####################################################
# for the LMMSE estimator: offline channel stats generation
# batch_size_LMMSE_1block = 1000
# LMMSE_offline_statsGenerate_block = 1
tau_w = 1000 # pilot length foe LMMSE
...
#######################################################################################################################
    ...
    V_subblocks_all_init, w_her_complex_init, F_complex_init, _, _ = GNN_output_proposed_approach(cell_UE_all_init,
                                                                                               MLP_w_in, MLP_V_in, MLP_F_in,
                                                                                               MLP_w_1, MLP_V_1, MLP1_1, MLP2_1, MLP3_1, MLP4_1, MLP5_1, MLP6_1,
                                                                                               MLP_w_2, MLP_V_2, MLP1_2, MLP2_2, MLP3_2, MLP4_2, MLP5_2, MLP6_2,
                                                                                               DNN_V_mapping_output, F_output, DNN_w_mapping_output,
                                                                                               num_user, subblocks_L, num_elements_irs, batch_size, num_antenna_bs, Pt_down_linear,
    ...
    'Attention! F^(0) and w^(0) cannot be followed by the frame structure.'
...
combined_A_test, _, _, _, _, _ = main_generate_channel(N_G, frames, N_w_using, sample_num_x, sample_num_y, num_antenna_bs, num_elements_irs, num_user, irs_Nh,
...
V_collect_no_direct = np.empty([1, num_elements_irs, 1, subblocks_L, frames], dtype=complex)
...
for frame in range(frames):
    ...
    'We collect the designed sensing vector {V} here since we want the first one but not the last one ---> please check the architecture diagram'
    # V_subblocks_all: tensor(batch_size, (num_elements_irs + 1), subblocks_L)
    # V_collect_no_direct: [1, num_elements_irs, 1, subblocks_L, frames]
    V_collect_no_direct[0, :, 0, :, frame] = V_fetch[0, 1:(num_elements_irs+1), :]
...
sio.savemat(file_path,
            {'H_lmmse_combine_designed_theta': H_lmmse_designed_w,
             ...
             'V_collect_no_direct': V_collect_no_direct})
