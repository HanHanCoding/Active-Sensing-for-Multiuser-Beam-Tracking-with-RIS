...
# # 2. Keras Dense Layer 已经包含了weights和bias -> bias set default means true!
...
# ----------------------------------------------------------------------------------------------------
'DNNs'
'注意，所有的DNN最后一层的linear(ouput) layer都定义在主程序中，因为这样方便定义输出tensor的size！'
# ----------------------------------------------------------------------------------------------------
...
def GNN_layer(x,w,V,MLP_w,MLP_V,MLP1,MLP2,MLP3,MLP4,MLP5,MLP6,num_user):
    # for input:
    ...
    """
        ...
        :return:x_out: dict{0:tensor(batch_size, hidden_size_GNN), 1:tensor(batch_size, hidden_size_GNN),..., K-1:tensor(batch_size, hidden_size_GNN)}
                ...
                V_out: tensor(batch_size, hidden_size_GNN)
    """
    w0 = MLP_w(w)
    V0 = MLP_V(V)
    ...
    for ii in range(num_user):
        ...
        if num_user == 1:
            x_concate = tf.concat([x[ii], w0, V0], axis=1)
        else:
            x_concate = tf.concat([x[ii], x_max, w0, V0], axis=1)
    ...
    'update the RIS node V ---> (A+C) block 3'
    tmp = []
    for ii in range(num_user):
        tmp.append(MLP5(x[ii]))
    x_mean = tf.reduce_mean(tmp, axis=0)
    x_concate = tf.concat([x_mean, V0], axis=1)
    V_out = MLP6(x_concate)

    return x_out, w_out, V_out
