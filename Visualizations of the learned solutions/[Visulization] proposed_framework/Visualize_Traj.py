...
from util_fun import compute_sum_rate, compute_min_rate, compute_pattern_gain_UL, compute_pattern_gain_DL, compute_array_response_irs, compute_array_response_Beamformer_allUE, compute_SINR_all_range, compute_SNR_SINR_table
...
SNR_SINR_table_allFrame = np.empty([num_user+1, num_user, frames])

SINR_perLocation_allFrame_UE_all = np.empty([sample_num_x * sample_num_y, 1, num_user, frames])

# combined_A_range_all: array [sample_num_x * sample_num_y, 1, num_antenna_bs, (num_elements_irs + 1), 1, moving_frames]
# "combined_A_range_all" is the channel w.r.t. the first blocks at each transmission frame
for frame in range(frames):
    'SNR_SINR_table at the location of the UEs - allFrame'
    # the true channel corresponding to the location of the UEs at the 0-block of each frame
    combined_A_UE_location = combined_A_test[0, :, :, :, N_w_using*frame] # [num_antenna_bs, (num_elements_irs + 1), num_user]
    SNR_SINR_table_allFrame[:, :, frame] = compute_SNR_SINR_table(combined_A_UE_location, w_collect[0, :, :, frame], F_collect[0, :, :, :, frame], num_user, noise_power_linear_down)
    ...
    for jj in range(sample_num_x):
        ...
        for kk in range(sample_num_y):
            ...
            'SINR map for each of the UEs --- fixing the channel per location, want to see whether beamformers is truely '
            SINR_perLocation_allFrame_UE_all[sample_num_y * jj + kk, 0, :, frame] = compute_SINR_all_range(combined_A_range_all[sample_num_y * jj + kk, 0, :, :, 0, frame], w_collect[0, :, :, frame], F_collect[0, :, :, :, frame], num_user, noise_power_linear_down)
...
path_SNR_SINR_table_allFrame = './Generate_MatFile_forVisualization/SNR_SINR_table_allFrame.mat'
sio.savemat(path_SNR_SINR_table_allFrame, {'SNR_SINR_table_allFrame': SNR_SINR_table_allFrame})


path_SINR_perLocation_allFrame_UE_all = './Generate_MatFile_forVisualization/SINR_perLocation_allFrame_UE_all.mat'
sio.savemat(path_SINR_perLocation_allFrame_UE_all, {'SINR_perLocation_allFrame_UE_all': SINR_perLocation_allFrame_UE_all})
