...
with tf.name_scope("channel_sensing"):
    ...
    w_her_complex, F_complex, mean_Estimated_error, Estimated_MISO_allUE_Tensor = GNN_output_proposed_approach(cell_UE_all, MLP_w_in, MLP_F_in,
...
for frame in range(frames):
    ...
    if frame == 0:
        ...
        with tf.Session() as sess:
            ...
            h_fetch, cell_fetch, V_fetch = sess.run([h_old_init_fetch_all, cell_old_init_fetch_all, V_subblocks_all], feed_dict=feed_dict_val)

    # here, we won't feed h_val frame per frame, but directly feed it as a whole
    feed_dict_val = {channel_combined_input: combined_A_test[:, :, :, :, frame * N_w_using:(frame + 1) * N_w_using], previous_hidden_state: h_fetch, previous_cell_state: cell_fetch}
